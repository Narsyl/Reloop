...
- `20260823210000_phase4c_tenant_guards` + `20260823230000_phase4c_reward_item_bindings`: triggers `fulfillment_marker_tenant_guard` (marker's integration must belong to the same organisation), `reward_binding_tenant_guard` (binding's reward item + integration same organisation, provider must match) and `integration_pairing_tenant_guard` (pairedIntegrationId must be a same-organisation RECHARGE integration). Triggers/functions are invisible to `prisma migrate diff`, so they never show as drift; keep them in mind when writing fixtures.
