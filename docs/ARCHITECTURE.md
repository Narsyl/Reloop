...
| 8 | No journeys until mapped | `Subscription.mappingStatus = UNMAPPED`, `currentJourneyId = null` when the current product/variant does not resolve; catalogue links (`productId/variantId`) are still set. |
...
**Known uncertainties to confirm on the first real import:** (a) `GET /orders` accepts `sort_by=id-asc` and `status=success` in 2021-11 (if not, the client's SCHEMA/VALIDATION error will surface in the run's error field and the parameter is removed); (b) `GET /token_information` shape — scopes are optional in the schema so a different shape only degrades write-verification to "unknown"; (c) `external_product_id.ecommerce` is present on `/products` items (products without it are skipped and counted in `productsSkipped`).
