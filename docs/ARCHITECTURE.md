...
## 26. Phase 4c — Shopify marker integration (23 Aug 2026): **built; catalogue + marker identity only; zero Recharge writes; no Shopify order access**

**Purpose (strict):** discover, create and verify the Shopify products/variants our `FulfillmentMarker`s point at. Not an order integration, not an embedded app, not a subscription source. Execution stays **our platform → Recharge `POST /onetimes` → Shopify order → Royal Mail**; this connector never edits Shopify orders and the "Shopify order created → we edit the order" race is not introduced. Recharge remains the subscription, lifecycle and upcoming-charge authority.

| Piece | Implementation |
|---|---|
| Provider / schema (migration `phase4c_shopify_markers`, additive; `phase4c_tenant_guards`, triggers only) | `IntegrationProvider.SHOPIFY`; `Integration.pairedIntegrationId` (a SHOPIFY integration is paired with the RECHARGE store whose one-times will reference its variants; `automationMode` stays OFF for Shopify). `FulfillmentMarker` += `shopifyIntegrationId`, `shopifyStatus`, `shopifyPublishedOnlineStore`, `shopifyPrice`, `shopifyInventoryTracked`, `shopifyHandle`, `lastVerifiedAt`, `verificationJson`, `rechargeCompatibility` (`UNVERIFIED` until the controlled Phase 6 test), `@@unique([shopifyIntegrationId, externalVariantId])`; `SubscriptionProgram.markerLabel / skuPrefix` (warehouse naming). **Canonical identity remains the numeric Shopify variant id** (`externalVariantId`, scoped to the Recharge integration) — never SKU or title. DB triggers (`fulfillment_marker_tenant_guard`, `integration_pairing_tenant_guard`) refuse a marker that references an integration / Shopify integration of another organisation (or a non-SHOPIFY one) and a pairing across organisations (or with a non-RECHARGE store); the migration fails closed if existing rows violated this. |
| Connector `lib/integrations/shopify/` | Admin GraphQL **2026-07**, custom-app Admin API token today (OAuth-issued tokens have the same shape → credential source is swappable without touching client/domain). `client.ts`: auth, timeout, retries/jitter on 429/5xx/network, cost-based throttle waits, Zod-validated responses (`SCHEMA_ERROR` on drift), typed errors, **mutation allowlist** (`productCreate`, `productUpdate`, `productVariantsBulkUpdate`, `publishablePublish`, `publishableUnpublish`) refusing anything else client-side (`FORBIDDEN_OPERATION`, incl. documents smuggling order/customer/fulfilment/… fields). `store.ts` (shop identity, granted scopes, capability probe — no write probe), `products.ts` (search by SKU/title/tag, read by product/variant id, `createMarkerProduct` = productCreate → productVariantsBulkUpdate {price, taxable false, inventoryPolicy CONTINUE, inventoryItem {sku, tracked false, requiresShipping true}} → publishablePublish (Online Store) → re-read; explicit `updateMarkerProduct`), `publications.ts`, `mapper.ts`, `types.ts` (`REQUIRED_SHOPIFY_SCOPES` = read/write products + read/write publications; `NOT_REQUESTED_AREAS`). |
| Domain | `lib/domain/integrations/shopify.ts`: test credentials (nothing saved), connect/reconnect (credentials encrypted per integration, AAD = integration id; refuses unless the capability report is usable; pairing validated tenant-side), re-check capabilities, connector factory (tenant-scoped), `findShopifyIntegrationForRecharge`. `lib/domain/markers/shopify.ts`: naming (`<label> <delivery>`, SKU `<PREFIX>-CYCLE-NN`, operator-overridable per programme), **missing-markers preview** (every programme × UPCOMING_RENEWAL milestone without a real marker — a placeholder counts as missing), duplicate detection (stored variant id → exact SKU → exact title → internal markers ⇒ `POSSIBLE_EXISTING_MARKER`, adopt instead or acknowledge), explicit **create** (target state: price 0.00, `UNLISTED`, Online Store published, inventory untracked, type "Fulfillment Marker", tag `subscription-ops-marker`; never DRAFT to hide), **adopt** (read-only), **verify** (read-only; issues `MISSING_IN_SHOPIFY / VISIBLE_TO_CUSTOMERS / DRAFT_OR_ARCHIVED / NOT_PUBLISHED_ONLINE_STORE / PRICE_NOT_ZERO / SKU_MISMATCH / TITLE_MISMATCH / INVENTORY_TRACKED / MISSING_MARKER_TAG`). Creating/adopting into a placeholder-bound milestone **re-points the same marker row** (placeholder cleared, previous identity removed, orphaned placeholder catalogue rows deleted, audited with the previous identity), then binds via `bindProgramMarker` (reward item must match). The connector never computes cycles, charges, one-times, actions or touches Recharge. |
| UI | Settings → Integrations: **Connect Shopify** dialog (test → capability panel → connect), Shopify cards, detail page with store identity, **capability panel** (requested/available vs never requested), markers table + verify. Products → Markers: **Missing fulfilment markers** table (marker · reward · programme/milestone · proposed SKU · Shopify state), **Generate markers… → explicit preview** (editable title/SKU, reward, price/status/publication, programme/milestone, duplicate check per row, adopt candidates, create one row at a time after confirmation — nothing is created until confirmed), per-programme naming, Shopify state + "Verify in Shopify" on each marker card. |
| Tests | `tests/unit/shopify-client.test.ts` (transport errors, retries, throttle wait, schema validation, allowlist refusal incl. smuggled fields, capability report, full create sequence and payloads, search/quoting, verification rules); `tests/integration/shopify-markers.test.ts` (fake Shopify endpoint + real DB: connect with encrypted creds / bad token / cross-org pairing refused, missing list + naming, duplicate → refused, create replaces placeholder → READY, adopt + verify issues, MISSING_IN_SHOPIFY, DB uniqueness per Shopify integration, tenant guards via `dbFor` and triggers, "only product/publication operations ever reach Shopify"). |

**Not done by design:** no bulk creation of the remaining markers (preview only until approved per programme), no Recharge write connector (`rechargeCompatibility` stays UNVERIFIED until Phase 6's controlled test), no webhooks, no Shopify order/customer/fulfilment access (scopes never requested), no OAuth yet (architecture ready for it). Pre-live gate still open: confirm the marker products are not customer-selectable in the Recharge customer portal / add-on settings before any LIVE execution.
