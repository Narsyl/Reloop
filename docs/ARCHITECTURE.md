...
## 26. Phase 4c (revised) — Shopify reward bindings (23 Aug 2026): **read-only Shopify; physical RewardItems own their variants; zero writes anywhere**

**Correction that reshaped this phase:** Whisk, Cup and Spoon already exist as real products in the Shopify store, and fulfilment/Royal Mail only needs to see the physical gift name ("Cup"), not a programme-specific alias ("Morning Magic 2"). The 26 synthetic marker products were therefore never created; the earlier same-day marker implementation (creation flows, write scopes, per-programme bindings, naming) was removed rather than kept.

**Execution model:**
```
RewardScheduleMilestone → RewardItem → RewardItemExternalBinding (reward × commerce integration)
                                     → existing Shopify ProductVariant (canonical externalVariantId)
```
Every programme that awards Cup resolves the same Cup variant. Recharge stays the subscription/lifecycle/one-time authority; the future `POST /onetimes` references the bound variant at £0.00 (`rechargeCompatibility` stays UNVERIFIED until the Phase 6 controlled test). The Shopify-order-edit race remains impossible: no Shopify write surface exists at all.

| Piece | Implementation |
|---|---|
| Schema (migration `phase4c_reward_item_bindings`, additive + column cleanup) | `RewardItemExternalBinding` (organizationId, rewardItemId, integrationId = SHOPIFY integration, provider, externalProductId/VariantId/Title/VariantTitle/Sku/Price/Status/Handle, requiresShipping, inventoryTracked, active, lastVerifiedAt, verificationJson, rechargeCompatibility; **unique (rewardItemId, integrationId)** and **unique (integrationId, externalVariantId)** — one binding per reward per store, one variant per reward). `AutomationAction.rewardItemId` (new actions reference the physical reward; `fulfillmentMarkerId` nullable = legacy). `Integration.encryptedAccessToken/accessTokenExpiresAt` (ephemeral token cache). Dropped: the never-populated marker Shopify columns and `SubscriptionProgram.markerLabel/skuPrefix`. **Legacy tables kept read-only for history:** `FulfillmentMarker` (discovered one-times, legacy rules/actions; AE's Cacao placeholder deactivated + audited) and `ProgramMilestoneMarker` (deactivated), scheduled for post-live cleanup. Tenant triggers: `fulfillment_marker_tenant_guard` (same-org integration), `reward_binding_tenant_guard` (reward item + integration same org, provider match), `integration_pairing_tenant_guard` (same-org RECHARGE pairing). |
| Authentication | Shopify Dev Dashboard app **client-credentials grant**: the operator enters shop domain + Client ID + Client secret in Settings → Integrations → Shopify (masked secret; Test → capability report → Connect). Durable credentials = client id/secret, AES-256-GCM per integration (AAD = id). The backend exchanges them at `POST /admin/oauth/access_token` (`grant_type=client_credentials`, ~24 h tokens), caches the token **encrypted** on the row (shared by web + jobs), refreshes ~5 min before expiry, and force-refreshes once on a 401. `authMode = CLIENT_CREDENTIALS` recorded in settings; a `TokenProvider` seam (`StaticTokenProvider`) keeps merchant OAuth possible later without touching the client or domain. Secret/token never in the browser, logs, Inngest payloads or ActivityLog metadata — only a Client ID hint ("abcd…wxyz"). |
| Connector `lib/integrations/shopify/` | Admin GraphQL 2026-07, **query-only by construction**: the client has no `mutate()` and refuses any mutation document with FORBIDDEN_OPERATION before any network call. Retries/jitter on 429/5xx/network, cost-throttle waits, Zod-validated responses. Required scope: **read_products** (empirically probed); optional read_publications (Online Store hint only); write scopes and every order/customer/fulfilment area are "never requested" — write scopes that happen to be granted are flagged so the operator can remove them. Reads: store identity, granted scopes, product search (`title:*x* OR sku:*x*` or explicit field queries), product/variant-id lookups. |
| Domain `lib/domain/rewards/bindings.ts` | `listRewardBindings` (reward × store → NEEDS_BINDING / BOUND / BLOCKED / INACTIVE with usage counts), `searchCatalog` (term or numeric/gid variant id), `bindRewardItem` (read variant → upsert the single binding row, re-point on rebind with audit + compatibility reset, friendly clash error), `unbindRewardItem` (refused while EXECUTING/ATTACHED actions exist; planner cancels PLANNED with REWARD_UNBOUND), `verifyRewardBinding` (re-read; issues MISSING_IN_SHOPIFY / DRAFT_OR_ARCHIVED blocking; NOT_REQUIRING_SHIPPING / INVENTORY_TRACKED / NOT_PUBLISHED_ONLINE_STORE / PRICED informational — real products are ACTIVE and priced; the one-time will be added at 0.00). |
| Resolver / planner / dry run | `resolveProgramRewards`: milestone → reward item → binding on the Shopify store paired with the programme's Recharge store; readiness reasons now `STORE_UNKNOWN / SHOPIFY_NOT_CONNECTED / REWARD_UNBOUND / BINDING_INACTIVE / BINDING_VARIANT_MISSING` (+ the schedule/milestone/programme ones). Planner keys on the **reward item** (`liveKey = journey:cycle:rewardItemId`, owner `c:<customer>:<programme>:<cycle>:<rewardItemId>`): re-binding Cup to another variant changes nothing (same physical reward — the variant is resolved at dry-run/execution time); changing a milestone's reward supersedes (`REWARD_CHANGED`); an unusable binding cancels (`REWARD_UNBOUND`). Dry run resolves the binding fresh and previews the exact payload: `external_variant_id = <bound Cup variant>`, `product_title = "Ceramic Cup"`, `price "0.00"`, properties `_subscription_ops_action` + `_subscription_ops_reward`; legacy rule-planned actions still resolve through their marker. |
| UI | Settings → Integrations → Shopify: connect dialog (domain, Client ID, masked Client secret, Test → least-privilege capability panel → Connect), detail page (store & auth incl. token expiry, capability panel, this store's reward bindings). Rewards page: **Reward fulfilment products** table (Reward · Shopify binding · Status) with read-only search → pick existing variant → confirm, Verify, Rebind, Unbind. Schedule detail: per-programme cells show reward → bound variant + readiness + live impact (no per-programme controls). Products → "Legacy markers" tab is read-only history. Upcoming/action/subscription pages show the reward item (legacy actions labelled). |
| Tests (258 green) | `tests/unit/shopify-client.test.ts`: token exchange (endpoint/body/expiry), caching + refresh margin + concurrent de-dup + shared cache, 401→refresh-once, mutation-document refusal, transport errors, throttle wait, capability report (read_products required, publications optional, write scopes flagged), search building/mapping. `tests/integration/reward-bindings.test.ts` (fake token endpoint + GraphQL, real DB): connect (bad secret, cross-org pairing refused, encrypted creds + encrypted token cache), cache reuse across connectors, revoked-token auto-refresh, search/bind/verify/rebind/unbind, DB uniqueness + tenant triggers + dbFor, "never saw a mutation". Planner/dry-run/reward-schedule suites rewritten on bindings; tenant-isolation covers `RewardItemExternalBinding`. |

**Ancient Extracts (23 Aug 2026):** the Cacao placeholder marker and its Morning Magic × Schedule B d2 binding were deactivated with audit rows (`LEGACY_MARKER_BINDING_RETIRED`, `LEGACY_MARKER_RETIRED`); lifecycle fingerprint unchanged (415/1004/360/870); integrity OK. Readiness now reads `SHOPIFY_NOT_CONNECTED` for every renewal milestone — the gate is: connect Shopify (client credentials, read_products) in Settings, bind **Whisk / Cup / Spoon** to their existing variants, verify, then the planner (DRY_RUN) plans the real population (MM d2 → Cup). **Still by design:** zero Recharge writes, zero Shopify writes, no webhooks (Phase 5), LIVE unreachable.

## 27. Phase 5 — Recharge webhooks (23 Aug 2026): **low-latency signals; reconcile-by-GET; zero store-data writes**

Webhook payloads are SIGNALS to reconcile, never a second lifecycle engine: every processed event does a targeted authenticated Recharge GET and feeds the result through the exact same import/normalisation/recalculation code the sync uses. Postgres stays the domain source of truth; Recharge stays the external authority; the 4-hourly incremental cron stays enabled as the reconciliation backstop.

| Piece | Implementation |
|---|---|
| Secret | The Recharge API client secret (separate from the API token) lives in the integration's encrypted credential blob (`{ apiToken, clientSecret }`, AAD = integration id) — set/replaced via a masked dialog on Settings → Integrations → Recharge (server action re-encrypts; never echoed, logged, or put in Inngest payloads / activity metadata). |
| Endpoint | `POST /api/webhooks/recharge/{integrationId}` (`app/api/webhooks/recharge/[integrationId]/route.ts`, node runtime). Raw body captured before parsing; `X-Recharge-Hmac-Sha256` validated with **Recharge's documented algorithm — a PLAIN `sha256(client_secret || exact raw body bytes)` hex digest, secret first (NOT a keyed HMAC despite the header name; docs.getrecharge.com/docs/webhooks-overview, verified 23 Aug 2026)** — constant-time compare, hex only, body bytes hashed untouched (no re-serialisation/whitespace normalisation). ACK is returned before any async work (~370 ms measured through the tunnel). Invalid → 401 (recorded as IGNORED, signatureValid=false, truncated excerpt, never processed, never queued). Tenancy resolves ONLY from the Integration row in the URL — payload contents cannot steer an event to another organisation. Valid → immutable `IntegrationEvent` (status RECEIVED) then Inngest dispatch then 200; no lifecycle work in the request. Secret missing → 503. |
| Dedupe | No trustworthy globally-unique delivery id in Recharge 2021-11 webhooks → deterministic fingerprint `sha256(topic + "\n" + rawBody)` unique per (integrationId, dedupeKey): byte-identical retries collapse at the door (200 duplicate:true); distinct updates to the same resource stay distinct events, and processing is intrinsically idempotent anyway (reconcile-by-GET + upserts on provider keys). |
| Processing | Inngest `integration-event-process` (trigger `integration/event.received`, concurrency 1 per integration, 3 retries) → `processIntegrationEvent` (`lib/domain/webhooks/process.ts`): order/* → `getOrder` → `upsertConnectorOrders` (the shared successful-order path: only success lines become evidence, purchase_item_id isolation, prepaid/shipment semantics untouched) → missing subscriptions fetched via `getSubscription` (order-before-subscription race closed) → orphan lines linked → `recalculateJourneysForSubscriptions` for affected subscriptions only; subscription/* → `getSubscription` → `upsertConnectorSubscriptions` (the shared subscription path: status, next charge, product/variant, catalogue derivation) → recalc that subscription. Vanished resources → IGNORED with detail. Then the EXISTING debounced planner event (`automation/plan.requested`, trigger WEBHOOK) when the integration mode ≠ OFF — the planner remains the only action writer and is idempotent (cancellation → SUBSCRIPTION_NOT_ACTIVE cancel; moved charge → in-place replan). |
| Redelivery sweep | Inngest cron `integration-event-redispatch` (*/15): re-dispatches RECEIVED events whose dispatch failed and PROCESSING strays (< 5 attempts). Double dispatch is harmless. |
| Registration | `RechargeClient.webhookAdmin(method, path)` is the ONLY non-GET Recharge surface in the platform, allowlisted to `^/webhooks(/\d+)?$` — every other path (and therefore /onetimes, /subscriptions, /orders) is refused before any HTTP. Domain `registerRechargeWebhooks` subscribes exactly six topics — **order/created, order/processed, subscription/created, subscription/updated, subscription/activated, subscription/cancelled** — at `{baseUrl}/api/webhooks/recharge/{id}`, re-points stale rows for our topics, leaves other topics alone; `unregister` removes only ours. Registration snapshot in settingsJson.webhooks. |
| UI | Recharge integration page → Webhooks: client secret ✓/missing (+ masked set/replace dialog), endpoint, per-topic registered chips, register/re-register/unregister, latest received/processed, 24h health (received/processed/failed/invalid-signature/pending), recent deliveries table (topic, resource id, status, attempts, truncated error — no payload dumps, no secrets). |
| Tests (274 green) | `tests/unit/webhook-signature.test.ts` (HMAC hex/base64, tamper, wrong secret, constant-time-safe lengths), `tests/unit/webhook-admin.test.ts` (allowlist refusal incl. /onetimes, dedupe determinism, exact topic list), `tests/integration/webhooks.test.ts` (endpoint: valid/duplicate/invalid/404/503, ids-only queue payloads, URL-only tenancy; processing with a fake authoritative connector: the user-example flow order→cycle 1→planner plans d2 reward, idempotent re-processing, stale-payload-vs-GET, order-before-subscription, non-success order = no evidence, cancelled → planner cancel + reactivation replan, sync-overlap duplicates nothing, cross-tenant refused). |

**Still by design:** no /onetimes write (unreachable — webhookAdmin path-allowlisted, `request` private), no Shopify writes, LIVE mode refused. Phase 6 adds the first allowlisted Recharge write.

## 28. Phase 6 — first controlled Recharge one-time write (24 Aug 2026): **one armed action; everything else still cannot write**

The goal is to prove exactly ONE allowlisted AutomationAction end-to-end (planned → fresh preflight → POST /onetimes → read-back → ATTACHED → later normal charge/order → Shopify → Royal Mail) and nothing more. General execution of planned actions remains impossible; LIVE mode remains refused.

| Piece | Implementation |
|---|---|
| Permission | The Recharge capability probe already derives one-time write from `write_subscriptions`; the integration page now shows **"One-time products — Read ✓ · Write ✓/Missing"** with guidance to update the token in Recharge when missing (no write is attempted without it). AE's token has it (`onetimes = read_write`, re-probed). |
| Write surface | `RechargeClient.createOnetime(body)` — POST hard-coded to `/onetimes`, **maxRetries forced to 0** (a write is never blind-retried), response Zod-validated (a 2xx with a malformed body throws SCHEMA_ERROR = uncertain, never success). `deleteOnetime(id)` — DELETE strictly `/onetimes/{numeric id}`, exists only for the explicit controlled-test rollback. No generic POST/PUT/DELETE exists; a test asserts the public client surface is exactly get/paginate/webhookAdmin/createOnetime/deleteOnetime. Connector adds typed `createOnetime`/`getOnetime`/`deleteOnetime`; `ConnectorOnetime` now carries `properties`. |
| Containment | `ControlledTestAuthorization`: one row arms ONE action (`armedKey` UNIQUE = integrationId while ARMED ⇒ the DATABASE guarantees at most one armed action per integration; tenant-guard trigger), 24 h expiry, single-use. `executeControlledTest` refuses without an ARMED row for exactly that action; consumes the authorization + claims PLANNED→EXECUTING in one transaction (a raced second process aborts); the planner/dry-run/Inngest paths contain no call to it. Accidentally executing the other 37 actions is structurally impossible. |
| Preflight | The full dry-run runs again immediately before the write (internal invariants incl. milestone readiness + verified binding, live subscription GET with exact `next_charge_scheduled_at == targetChargeDate` equality, fresh one-time scan). Any blocker → abort, action stays PLANNED, authorization stays ARMED. A pre-existing matching one-time → **ADOPT** (no POST). |
| Payload | `lib/domain/actions/payload.ts` `buildOnetimeBody` — the SAME builder produces the dry-run preview and the executed body (exact-date pinning, never `add_to_next_charge`; price "0.00", quantity 1; `_subscription_ops_action` + `_subscription_ops_reward` properties). |
| Outcomes | Definite 4xx (validation/permission/auth/not-found) → FAILED with the provider error, no retry. Ambiguous (network death / 5xx / rate-limit / malformed 2xx) → action stays EXECUTING and is resolved by READ reconciliation (`findOurOnetime`: the action property is the primary identity, exact variant+date the fallback — a same-variant one-time on a different date is never falsely adopted); a single creation retry is permitted only after reconciliation positively proves absence; a second ambiguity stays EXECUTING for the operator. Success → immediate authoritative `GET /onetimes/{id}` read-back verifying id, address, date, product, variant, quantity 1, price 0.00, properties — **ATTACHED only when every field matches**; any mismatch → FAILED + CRITICAL exception (identity kept so rollback can target it). |
| Compatibility | On a verified ATTACHED, ONLY the tested variant's `RewardItemExternalBinding` is promoted UNVERIFIED→VERIFIED, recording verifiedAt/integration/variant/action/one-time id inside `verificationJson.rechargeVerification`. Whisk/Spoon stay UNVERIFIED until tried. Rollback demotes it again if the evidence points at the rolled-back one-time. |
| Rollback | `rollbackControlledTest(actionId, reason)` — explicit operator action only: requires the consumed authorization + stored external id, re-reads the live one-time, verifies it carries our identity, DELETEs exactly that one-time, confirms it is gone, cancels the action (keys freed) and demotes the promoted compatibility. No generic deletion UI/API exists. |
| Tests (15 new; suite green) | arming single/duplicate/raw-insert/expiry/non-PLANNED; unarmed & DRY_RUN & LIVE cannot write; moved charge / cancelled sub / no charge refuse with nothing written; happy path (one POST, previewed payload, read-back, ATTACHED, keys retained, single-use); planner leaves ATTACHED alone; definite 4xx → FAILED; timeout-after-create → reconciliation adopts (1 POST total); timeout-without-create → proven absence → exactly one retry; wrong-date same-variant never falsely adopted + true adopt path (0 POSTs); malformed 2xx attaches only via reconciliation; read-back tamper (price/variant) → FAILED + exception; rollback; tenant isolation + DB trigger; client mutation-surface lockdown. |

**Process:** implementation + tests first, then STOP: present 3 suitable live candidates (fresh provider state + exact payloads) — the operator explicitly selects ONE action, which is then armed and executed once, reported in full, and everything stops again. The remaining 37 actions are not enabled.
